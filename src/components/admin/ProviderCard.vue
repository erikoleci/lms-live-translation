  ...
  <v-card rounded="xl" :class="{ 'provider-card--disabled': !provider.enabled }" class="provider-card">
    ...
    <v-card-text class="pt-0">
      ...
      <v-row class="text-caption text-medium-emphasis">
        <v-col cols="6">
          <v-icon size="12" class="mr-1">mdi-sort-numeric-ascending</v-icon>
          Priority: <strong>{{ provider.priority }}</strong>
        </v-col>
        <v-col cols="6">
          <v-icon size="12" class="mr-1">mdi-timer-outline</v-icon>
          Timeout: <strong>{{ provider.timeoutMs / 1000 }}s</strong>
        </v-col>
        <v-col cols="6" v-if="provider.costLimit">
          <v-icon size="12" class="mr-1">mdi-currency-usd</v-icon>
          Limit: <strong>${{ provider.costLimit }}/mo</strong>
        </v-col>
        <v-col cols="6" v-if="provider.fallbackProviderCode">
          <v-icon size="12" class="mr-1">mdi-backup-restore</v-icon>
          Fallback: <strong>{{ provider.fallbackProviderCode }}</strong>
        </v-col>
      </v-row>
    </v-card-text>

    <v-card-actions class="px-4 pb-4 pt-0">
      <v-chip size="x-small" :color="typeColor" variant="tonal" class="font-weight-bold">
        {{ provider.providerType }}
      </v-chip>
      <v-chip
        v-if="provider.priority === 1"
        size="x-small"
        color="amber"
        variant="tonal"
      ...
      <v-spacer />
      <v-btn size="small" variant="text" prepend-icon="mdi-pencil" @click="emit('edit', provider.id)">
        Edit
      </v-btn>
    </v-card-actions>
  </v-card>
...
const emit = defineEmits<{ toggle: [id: string]; edit: [id: string] }>()
