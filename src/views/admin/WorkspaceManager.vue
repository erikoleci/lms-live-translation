<template>
  <v-container fluid class="pa-6">
    <div class="d-flex align-center mb-6">
      ...
      <v-spacer />
      <v-btn color="primary" variant="flat" prepend-icon="mdi-plus" @click="showCreateWs = true">
        New Workspace
      </v-btn>
    </div>
    ...
    <v-row>
      ...
      <!-- Classes for selected workspace -->
      <v-col cols="12" md="8">
        <v-card rounded="xl" elevation="0" border>
          <v-card-title class="text-body-1 font-weight-bold pt-4 px-4 d-flex align-center">
            ...
            <v-spacer />
            <v-btn v-if="selectedWs" size="small" color="primary" variant="tonal" prepend-icon="mdi-plus" @click="showCreateClass = true">
              Add Class
            </v-btn>
          </v-card-title>
            ...
            <v-card-text class="px-4 pb-4">
              <v-row>
                <v-col v-for="cls in selectedClasses" :key="cls.id" cols="12" sm="6">
                  <v-card rounded="xl" elevation="0" border class="class-card pa-4">
                    ...
                    <div class="stats-row">
                      <div class="stat">
                        <p class="text-h6 font-weight-bold text-primary">{{ classStats(cls.id).sessionCount }}</p>
                        <p class="text-caption text-medium-emphasis">Sessions</p>
                      </div>
                      <div class="stat">
                        <p class="text-h6 font-weight-bold text-success">{{ formatHours(classStats(cls.id).totalMinutes) }}</p>
                        <p class="text-caption text-medium-emphasis">Total Hours</p>
                      </div>
                      <div class="stat">
                        <p class="text-h6 font-weight-bold text-orange">{{ classStats(cls.id).participantCount }}</p>
                        <p class="text-caption text-medium-emphasis">Participants</p>
                      </div>
                    </div>
                  </v-card>
                </v-col>
    ...
    <v-dialog v-model="showCreateWs" max-width="440" persistent>
      <v-card rounded="xl">
        ...
        <v-card-actions class="px-6 pb-6 gap-2">
          ...
          <v-btn color="primary" variant="flat" @click="createWorkspace">Create</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="showCreateClass" max-width="440" persistent>
      <v-card rounded="xl">
        ...
        <v-card-actions class="px-6 pb-6 gap-2">
          ...
          <v-btn color="teal" variant="flat" @click="createClass">Create</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
function createWorkspace() {
  ...
  const ws = store.createWorkspace(wsForm.value)
  selectedWs.value = ws
...
function doDelete() {
  if (deletingClass.value) store.deleteClass(deletingClass.value.id)
...
.class-card:hover { box-shadow: 0 4px 20px rgba(0,0,0,0.08) !important; }
.stats-row { display: flex; gap: 8px; }
.stat { flex: 1; text-align: center; padding: 8px; background: rgba(0,0,0,0.03); border-radius: 10px; }
</style>
