<template>
  <div class="active-session-page" :class="{ 'dark-page': uiStore.darkMode }" v-if="session">
    ...
    <!-- TOP BAR -->
    <div class="session-topbar px-2 px-sm-4">
      <!-- Mobile sidebar toggle -->
      <v-btn v-if="smAndDown" icon="mdi-menu" variant="text" size="small" @click="showSidebar = !showSidebar" class="mr-1" />
      <v-btn v-else icon="mdi-arrow-left" variant="text" size="small" to="/teacher" />

      <div class="flex-grow-1 min-w-0 mx-2">
        <div class="d-flex align-center gap-1 gap-sm-2">
          <h1 class="text-body-2 text-sm-body-1 font-weight-bold text-truncate">{{ session.title }}</h1>
          <StatusChip :status="session.status" />
        </div>
        <p class="text-caption text-medium-emphasis d-none d-sm-block">{{ session.courseName }}</p>
      </div>

      <div class="d-flex align-center gap-1 gap-sm-2">
        <div class="d-none d-sm-flex align-center gap-1 text-caption">
        ...
        <v-btn
          ...
          size="small"
          :icon="smAndDown ? undefined : undefined"
          rounded="lg"
          @click="qrFloat = !qrFloat"
          class="px-2 px-sm-3"
        >
          <v-icon size="16">mdi-qrcode</v-icon>
          <span class="d-none d-sm-inline ml-1">QR</span>
        </v-btn>
        ...
        >
          ...
          <span class="d-none d-sm-inline ml-1">Save</span>
        </v-btn>

        <v-btn :icon="uiStore.darkMode ? 'mdi-weather-sunny' : 'mdi-weather-night'" variant="text" size="small" @click="uiStore.toggleDarkMode()" />
        <v-btn v-if="smAndDown" icon="mdi-arrow-left" variant="text" size="small" to="/teacher" />
      </div>
    </div>
    ...
    <transition name="fade-back">
      <div v-if="smAndDown && showSidebar" class="sidebar-backdrop" @click="showSidebar = false" />
    </transition>
    ...
    <div class="session-body">
      ...
      <aside
        ...
        :class="{ 'session-sidebar--open': !smAndDown || showSidebar }"
      >
        <div class="sidebar-scroll">

          <!-- Back button on mobile inside sidebar -->
          <div v-if="smAndDown" class="d-flex align-center justify-space-between mb-3 px-1">
            <span class="text-caption font-weight-bold text-medium-emphasis text-uppercase">Controls</span>
            <v-btn icon="mdi-close" size="x-small" variant="text" @click="showSidebar = false" />
          </div>
          ...
          <v-card rounded="xl" elevation="0" border class="mb-3">
            ...
            <v-card-text class="px-3 pb-3">
              ...
              <div class="mic-btn-row">
                <v-btn v-if="!sessionStore.micActive" color="success" variant="flat" size="large" class="flex-grow-1" prepend-icon="mdi-microphone" :disabled="session.status !== 'ACTIVE'" :loading="micLoading" @click="startMic">Start</v-btn>
                <template v-else>
                  <v-btn :color="sessionStore.isMuted ? 'warning' : 'default'" :variant="sessionStore.isMuted ? 'flat' : 'tonal'" size="large" class="flex-grow-1" :prepend-icon="sessionStore.isMuted ? 'mdi-microphone-off' : 'mdi-pause'" @click="sessionStore.toggleMute()">
          ...
          <v-card rounded="xl" elevation="0" border>
            ...
            <v-card-text class="px-3 pb-3 d-flex flex-column gap-2">
              ...
              <div class="info-row">
                <span class="text-caption text-medium-emphasis">Join code</span>
      ...
      <main class="transcript-main">
        ...
        <!-- Transcript header -->
        <div class="transcript-header px-3 px-sm-4 py-2 d-flex align-center gap-2">
          ...
          <v-btn size="x-small" variant="text" icon="mdi-download" :disabled="!segments.length" @click="downloadTranscript" />
        </div>
        <v-divider />
    ...
    <transition name="qr-pop">
      ...
      >
        <div class="qr-title-bar">
          ...
          <span class="text-caption font-weight-bold">Join — {{ session.joinCode }}</span>
          <v-spacer />
        ...
        <div v-if="!qrMinimized" class="qr-float-body">
          ...
          <v-chip color="primary" variant="flat" class="font-weight-bold join-code-chip mt-3">{{ session.joinCode }}</v-chip>
...
const floatStyle = computed(() => ({
  top: qrPos.value.y + 'px',
  left: Math.max(8, Math.min(window.innerWidth - 260, qrPos.value.x)) + 'px',
}))
...
function downloadTranscript() {
  ...
  const lines = [
    `Session: ${sess.title}`, `Course: ${sess.courseName}`,
    `Date: ${new Date(sess.startedAt ?? sess.createdAt).toLocaleDateString()}`,
...
const micStatusClass = computed(() => !sessionStore.micActive ? 'ring--idle' : sessionStore.isMuted ? 'ring--muted' : 'ring--live')
const micIconColor = computed(() => !sessionStore.micActive ? 'grey' : sessionStore.isMuted ? 'warning' : 'success')
...
onMounted(() => {
  ...
  if (session.value?.status === 'ACTIVE') { wsStatus.value = 'CONNECTED'; startSim() }
})
onUnmounted(() => {
  stopSim(); stopMicHw(); sessionStore.setActiveSession(null)
  window.removeEventListener('mousemove', onDrag)
  window.removeEventListener('mouseup', endDrag)
})
...
async function handleStart() {
  actionLoading.value = true; await new Promise(r => setTimeout(r, 800))
  sessionStore.updateSessionStatus(sessionId, 'ACTIVE'); wsStatus.value = 'CONNECTED'; actionLoading.value = false
}
async function handlePause() {
  actionLoading.value = true; await new Promise(r => setTimeout(r, 400))
  sessionStore.updateSessionStatus(sessionId, 'PAUSED'); actionLoading.value = false
}
async function handleResume() {
  actionLoading.value = true; await new Promise(r => setTimeout(r, 400))
  sessionStore.updateSessionStatus(sessionId, 'ACTIVE'); actionLoading.value = false
}
async function handleEnd() {
  actionLoading.value = true; stopSim(); stopMicHw()
  await new Promise(r => setTimeout(r, 600))
  sessionStore.updateSessionStatus(sessionId, 'ENDED'); wsStatus.value = 'DISCONNECTED'; actionLoading.value = false
}
</script>
...
.session-topbar { display: flex; align-items: center; background: white; min-height: 56px; gap: 4px; }
.dark-page .session-topbar { background: #1E1E2E; }

.session-body { display: flex; flex: 1; overflow: hidden; position: relative; }
...
@media (min-width: 960px) {
  .session-sidebar { transform: none !important; }
}
...
.fade-back-enter-from, .fade-back-leave-to { opacity: 0; }
.fade-back-enter-active, .fade-back-leave-active { transition: opacity 0.2s; }
...
/* Transcript main */
.transcript-main { flex: 1; display: flex; flex-direction: column; overflow: hidden; min-width: 0; }
.teacher-caption-bar { padding: 12px 16px; background: rgba(21,101,192,0.04); border-bottom: 2px solid rgba(21,101,192,0.1); min-height: 76px; transition: all 0.3s; }
...
.teacher-live-text { font-size: 16px; font-weight: 600; line-height: 1.5; color: #555; margin: 0; }
...
.transcript-header { background: white; min-height: 44px; }
...
/* QR float */
.qr-float-window {
  position: fixed; z-index: 500; width: 252px;
...
.qr-title-bar { display: flex; align-items: center; padding: 6px 8px 6px 12px; background: rgba(21,101,192,0.06); border-bottom: 1px solid rgba(0,0,0,0.07); cursor: move; }
.qr-float-body { display: flex; flex-direction: column; align-items: center; padding: 16px; gap: 4px; }
.qr-canvas-wrap { padding: 10px; background: white; border-radius: 12px; box-shadow: 0 2px 12px rgba(0,0,0,0.1); }
.join-code-chip { letter-spacing: 4px !important; font-size: 16px !important; }
